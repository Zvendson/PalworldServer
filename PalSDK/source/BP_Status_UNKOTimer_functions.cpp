#include "PalServer/Basic.hpp"

#include "PalServer/BP_Status_UNKOTimer_classes.hpp"
#include "PalServer/BP_Status_UNKOTimer_parameters.hpp"


namespace PalServer
{

// Function BP_Status_UNKOTimer.BP_Status_UNKOTimer_C.ExecuteUbergraph_BP_Status_UNKOTimer
// (Final, UbergraphFunction, HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_Status_UNKOTimer_C::ExecuteUbergraph_BP_Status_UNKOTimer(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Status_UNKOTimer_C", "ExecuteUbergraph_BP_Status_UNKOTimer");

	Params::BP_Status_UNKOTimer_C_ExecuteUbergraph_BP_Status_UNKOTimer Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Status_UNKOTimer.BP_Status_UNKOTimer_C.OnBeginStatus
// (Event, Public, BlueprintEvent)

void UBP_Status_UNKOTimer_C::OnBeginStatus()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Status_UNKOTimer_C", "OnBeginStatus");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_Status_UNKOTimer.BP_Status_UNKOTimer_C.OnEndStatus
// (Event, Public, BlueprintEvent)

void UBP_Status_UNKOTimer_C::OnEndStatus()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Status_UNKOTimer_C", "OnEndStatus");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_Status_UNKOTimer.BP_Status_UNKOTimer_C.カスタムイベント_0
// (BlueprintCallable, BlueprintEvent)

void UBP_Status_UNKOTimer_C::_________________________0()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Status_UNKOTimer_C", "カスタムイベント_0");

	UObject::ProcessEvent(Func, nullptr);
}

}

